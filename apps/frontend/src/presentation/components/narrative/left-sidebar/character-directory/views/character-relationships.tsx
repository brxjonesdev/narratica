...
import { Delete, EditIcon, UserPlus } from "lucide-react"
...
export default function CharacterRelationships({
  ...
  const [newRelationship, setNewRelationship] = useState<{ selectedCharacter: Character | null, description: string, type: RelationshipType | null}>({
  ...
  const uniqueRelationships = Object.values(relationshipLabels).reduce((acc, type) => {
    const relationships = character[type as RelationshipType] || []
    return acc + relationships.length
  }
  , 0)

  if (filteredCharacters.length === 0) {
    return <div className="p-4 text-muted-foreground">
  ...
  const handleAddRelationship = (type: RelationshipType, characterId: string, description: string) => {
    onChange(type, [...(character[type as RelationshipType] || []), { id: characterId, name: newRelationship.selectedCharacter?.name || "", description: description }])
                  ...
                  <DropdownMenuSubContent className="w-56">
                    ...
                    .map((char) => (
                      <DropdownMenuItem
                        ...
                        onClick={
                          ()=> {
                            setNewRelationship({ selectedCharacter: char, description: "", type: type as RelationshipType })
              ...
              <div key={type} className="mt-4">
                ...
                <div>
                  {character[type as RelationshipType]?.map((relationship) => {
                    const relatedCharacter = availableCharacters.find((c) => c.id === relationship.id)
                    return (
                      <Card key={relationship.id} className="mt-2 px-1 py-2 space-y-0 flex justify-between">
                        <CardHeader className="px-2 py-1">
                          <CardTitle className="font-medium truncate text-sm px-3">{relatedCharacter?.name}</CardTitle>
                          <CardDescription className="text-xs text-muted-foreground truncate tracking-wider font-semibold">
                            ...
                            onChange={(value: string) => {
                              const updatedRelationships = character[type as RelationshipType]?.map((rel) =>
                                rel.id === relationship.id ? { ...rel, description: value } : rel,
                              ) || [];
                  ...
                  <Button
                    onClick={() =>{
                      if (newRelationship.selectedCharacter?.id) {
                        handleAddRelationship(newRelationship.type as RelationshipType, newRelationship.selectedCharacter.id, newRelationship.description)
