...
interface OutlineRepository {
  ...
  addLocationToScene: (
    sceneID: string,
    location: Partial<NarrativeLocation>,
  ) => Promise<Result<{ ok: boolean }, string>>;
...
export const outlineRepository: OutlineRepository = {
  ...
  outlines(where: $where, limit: $limit) {
    ...
    try {
      const response = await GraphQLFetcher<{ data: { outlines: Outline[] } }>(
        FETCH_PLOT,
        {
          where: {
    ...
    try {
      ...
      if (!outline || outline === undefined) {
        ...
        const response = await GraphQLFetcher<{
          data: { createOutlines: { outlines: Outline[] } };
        }>(INIT_OUTLINE, {
          input: [
            {
              id: newOutline.id,
      ...
      const response = await GraphQLFetcher<{
        data: { updateOutlines: { info: { nodesCreated: number } } };
      }>(CREATE_ACT, {
        where: {
          narrativeID_EQ: narrativeID,
        },
  ...
  updateActs(where: $where, update: $update) {
    ...
    try {
      const response = await GraphQLFetcher<{
        data: { updateActs: { info: { nodesCreated: number } } };
      }>(UPDATE_ACT, {
        where: {
          id_EQ: actID,
        },
...
`;
    ...
    try {
      const response = await GraphQLFetcher<{
        data: { deleteActs: { nodesDeleted: number } };
      }>(DELETE_ACT, {
        where: {
          id_EQ: actID,
        },
  ...
  async modifySceneByID(sceneID: string, scene: Partial<Scene>) {
    ...
    try {
      const response = await GraphQLFetcher<{
        data: { updateScenes: { info: { nodesCreated: number } } };
      }>(UPDATE_SCENE, {
        where: { id_EQ: sceneID },
        update,
      });
  ...
  deleteScenes(where: $where) {
    ...
    try {
      const response = await GraphQLFetcher<{
        data: { deleteScenes: { nodesDeleted: number } };
      }>(DELETE_SCENE, {
        where: {
          id_EQ: sceneID,
        },
  ...
  updateScenes(where: $where, update: $update) {
    ...
    try {
      const response = await GraphQLFetcher<{
        data: { updateScenes: { info: { relationshipsCreated: number } } };
  ...
  updateScenes(where: $where, update: $update) {
    ...
    try {
      const response = await GraphQLFetcher<{
        data: { updateScenes: { info: { relationshipsDeleted: number } } };
