...
export function useManuscript() {
  ...
  const deleteAct = async (actID: string) => {
    ...
    const result = await deleteActFromOutline(
      id as string,
      actID,
      deletedAct?.chapters.map((chapter) => chapter.id) || [],
      deletedAct?.chapters.flatMap((chapter) =>
